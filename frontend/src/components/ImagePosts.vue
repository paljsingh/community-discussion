        ...
        <v-card dark>
            ...
            >
                <template v-slot:[`item.file`]="{item}">
                    <v-img class="imagepost"
                        :lazy-src="'http://localhost:5002/api/v1/image/' + item._id"
                        :src="'http://localhost:5002/api/v1/image/' + item._id">
                    </v-img>
                </template>
            </v-data-table>
        </v-card>
    ...
    export default {
        ...
        data: function() {
            return {
                ...
                apiUrl: process.env.VUE_APP_COMMUNITIES_IMAGES_API_ENDPOINT,
